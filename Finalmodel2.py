...
# Declare global model variables
posenet_model = None
mp_hands = None
mp_drawing = None
wav2vec2_model = None
...
def load_models():
    global posenet_model, mp_hands, mp_drawing, wav2vec2_model, processor, pronunciation_pipe, t5_model, tokenizer
    
    try:
        # Load PoseNet model for posture detection (using TensorFlow Hub)
        posenet_model_url = "https://tfhub.dev/google/movenet/singlepose/lightning/4"
        posenet_model = hub.load(posenet_model_url)
        logger.info("PoseNet model loaded successfully.")
    except Exception as e:
        logger.error(f"Error loading PoseNet model: {e}")
    ...
    try:
        # Load Wav2Vec2 model and processor for sequence classification (filler word detection)
        num_labels = 6
    ...
    try:
        ...
        logger.info("Pronunciation evaluation pipeline initialized.")
    except Exception as e:
        logger.error(f"Error initializing pronunciation pipeline: {e}")
    ...
    try:
        ...
        logger.info("T5 model for grammar correction loaded successfully.")
    except Exception as e:
